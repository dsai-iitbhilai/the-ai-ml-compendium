# 10 — Interview Prep

> **Goal:** Prepare for ML engineering and data science interviews — covering ML theory, system design, coding, and behavioral questions.

## Sections

| Section | Description | Level |
|---|---|---|
| [ML Interview Questions](./ml-interview-questions.md) | Common ML questions with resources | All levels |
| [System Design for ML](./system-design-for-ml.md) | Designing ML systems (modeling and infrastructure) | Senior / Staff |
| [Coding Practice](./coding-practice.md) | LeetCode, algorithm prep, ML coding rounds | All levels |

## Interview Types by Role

| Role | ML Theory | System Design | Coding | Behavioral |
|---|---|---|---|---|
| **Data Scientist** | High | Medium | Low-Med | High |
| **ML Engineer** | High | High | Medium | Medium |
| **Research Scientist** | Very High | Low | Low-Med | Medium |
| **Applied Scientist** | High | High | Medium | Medium |
| **AI Product Manager** | Medium | Low | None | High |

## General Interview Resources

| Category | Title | Description | Level |
|---|---|---|---|
| 📘 Docs | [Ace the Data Science Interview — Book](https://example.com/ace-ds) | Comprehensive Q&A for DS interviews | All levels |
| 🎥 Video | [ML Interview Prep Playlist — YouTube](https://example.com/ml-interview-playlist) | Mock interviews and topic reviews | Intermediate |
| 📰 Blog | [ML System Design Interview Guide — ByteByteGo](https://example.com/bytebytego-ml) | Structured approach to ML design questions | Advanced |

---

> **Last Reviewed:** 2026-06
